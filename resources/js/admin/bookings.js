...
function getMonthSchedule (a_date) {
  ...
  var response = $.ajax({
    type: 'POST',
    url: '/api/calendarevent/getschedule',
    data: { start: month_start, end: month_end },
    dataType: 'json',
    async: false,
    success: function (msg) {
      if (msg.status == 'fail') {
...
function getEventBookings (ce_id) {
  ...
  $.ajax({
    ...
    error: function (xhr) {
      ...
      $('#modal_calendarevent_body').html(xhr.responseJSON.msg)
...
jQuery(document).ready(function ($) {
  ...
  case '/admin/bookings/calendarevent':
    ...
    case 'bkg_index':
      getEventBookings(month_schedule[i].id)
